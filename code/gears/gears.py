...
from .utils import loss_fct, ising_loss_fct, uncertainty_loss_fct, parse_any_pert, \
...
class GEARS:
    ...
    def tunable_parameters(self):
        ...
        return {'hidden_size': 'hidden dimension, default 64',
                ...
                'ising_lambda': 'regularization term to balance ising loss and other losses, default 1', 
                'ising_temperature': 'temperature of ising model for ising loss ',
                'ising_cutoff': 'cutoff used to calculate ising states of changes to gene expression, default 1e-1', 
                'ising_graph': 'adjacency matrix for ising model', 
               }
    ...
    def model_initialize(self, hidden_size = 64,
        ...
        self.config = {'hidden_size': hidden_size,
                       ...
                       'ising_temperature': ising_temperature, 
            ...
            for step, batch in enumerate(train_loader):
                ...
                if self.config['uncertainty']:
                    ...
                    loss = loss_fct(pred, y, batch.pert,
                                  ...
                                  dict_filter = self.dict_filter,
                                  direction_lambda = self.config['direction_lambda'],
                                  ising = self.config['ising'], 
                                  ising_lambda = self.config['ising_lambda'],
                                  ising_graph = self.config['ising_graph'],
                                  ising_cutoff = self.config['ising_cutoff'],
                                  ising_temperature = self.config['ising_temperature'],
                                  )
                loss.backward()
